...
export const createReminderSchema = z
  .object({
    ...
    interval: z.number().int().min(1, "Interval must be at least 1").max(365, "Interval must be at most 365").optional(),
    dayOfWeek: z.number().int().min(0).max(6, "Day of week must be 0 (Sunday) - 6 (Saturday)").optional(),
...
export const updateReminderSchema = z.object({
  ...
  frequency: reminderFrequencySchema.optional(),
  interval: z.number().int().min(1, "Interval must be at least 1").max(365, "Interval must be at most 365").optional(),
  dayOfWeek: z.number().int().min(0).max(6, "Day of week must be 0 (Sunday) - 6 (Saturday)").nullable().optional(),
...
export const reminderQuerySchema = z.object({
  ...
  sortBy: z.enum(["createdAt", "startDate", "nextTriggerDate", "title"]).optional().default("createdAt"),
